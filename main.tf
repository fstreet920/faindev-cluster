...
locals {
  cluster_name = "faindev-dev-eks-${random_string.suffix.result}"
}
...
module "eks" {
  ...
  # add ingress rull so istio-injection will work
  node_security_group_additional_rules = {
    ingress_from_eks = {
      description = "Allow istio webhook access"
      protocol    = "tcp"
      from_port   = 15017
      to_port     = 15017
      type        = "ingress"
  ...
  # instances for gaming microservices.
  eks_managed_node_groups = {
    ...
    two = {
      ...
      # taint these nodes for game microservices
      taints = {
        dedicated = {
          key    = "dedicated"
          value  = "gameServices"
